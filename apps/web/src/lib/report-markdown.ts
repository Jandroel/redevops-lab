...
export function createReportMarkdown(report: DevOpsReport): string {
  ...
  const signals = report.analysis?.devopsSignals.length
    ? report.analysis.devopsSignals
        ...
        .join("\n")
    : `- ${label(language, "No disponible en este reporte.", "Not available in this report.")}`;
  const categories = report.score.categories.map((category) => formatCategory(category, language)).join("\n\n");
  const strengths =
  ...
  const findings = report.findings
    ...
    .join("\n");
  const checklist = formatProductionChecklist(report.productionChecklist, language);
  const learningPath = formatLearningPath(report.learningPath, language);
  const labs = formatLabs(report.labs, language);
