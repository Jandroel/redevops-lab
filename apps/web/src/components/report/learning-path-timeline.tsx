...
export function LearningPathTimeline({ steps, initialVisibleCount = 5 }: LearningPathTimelineProps) {
  const [expanded, setExpanded] = useState(false);
  const visibleSteps = expanded ? steps : steps.slice(0, initialVisibleCount);
  const remainingSteps = Math.max(steps.length - visibleSteps.length, 0);
    ...
    <section className="rounded-lg border border-devops-border bg-slate-950/55 p-6">
      ...
      {steps.length ? (
        <>
                ...
                >
                  <div className="flex flex-col items-center">
                    ...
                    {index < visibleSteps.length - 1 ? (
                      <span className="mt-2 h-full w-px bg-devops-border" />
                    ) : null}
                  </div>
          ...
          {steps.length > initialVisibleCount ? (
            <div className="mt-6 flex justify-center">
              <Button variant="secondary" onClick={() => setExpanded((value) => !value)}>
                {expanded ? "Show fewer steps" : `Show ${remainingSteps} more steps`}
              </Button>
            </div>
          ) : null}
        </>
