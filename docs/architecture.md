...
- `packages/analyzer`: GitHub repository analyzer. It validates URLs, reads repository trees, selects bounded high-value text files, parses configuration and documentation, and detects stack and DevOps evidence.
...
The analyzer form routes from `apps/web` to `apps/api`. The backend validates the request, calls GitHub for public repository metadata and the recursive tree, reads a bounded allowlist of public text files, runs deterministic detectors, calculates a rule-based score, and generates educational guidance before returning a typed `DevOpsReport`.
...
User GitHub URL
  ...
  -> GitHub REST API
  -> packages/analyzer
  -> bounded content selection and parsing
  -> packages/scoring
  -> packages/learning
...
- safely fetching a bounded allowlist of public text files
- parsing package scripts, workflow triggers and steps, Docker and Compose practices, README coverage, and env placeholder safety
...
- generating up to 20 production-readiness checklist items from signals and content-backed score rules
...
The learning engine is deterministic. It consumes bounded content evidence produced by the analyzer, but does not fetch content itself, use AI, clone repositories, write to repositories, create GitHub issues, or persist data.
